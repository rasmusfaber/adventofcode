...
fun <P> bfsWithLength(graph: DirectedGraph<P>, start: P, predicate: (P) -> Boolean) = bfsWithLength(graph, start).filter { predicate(it.first) }
...
fun <P> bfsWithPath(graph: DirectedGraph<P>, start: P, predicate: (P) -> Boolean) = bfsWithPath(graph, start).filter { predicate(it.last()) }
...
fun <P> dfsWithPath(graph: DirectedGraph<P>, start: P, predicate: (P) -> Boolean) = dfsWithPath(graph, start).filter { predicate(it.last()) }
...
fun <P> DirectedWeightedGraph<P>.withExtraConnections(extra: Map<P, Collection<Edge<P>>>) = DirectedWeightedGraphWithExtraConnections(this, extra)

class DirectedWeightedGraphWithExtraConnections<P>(private var inner: DirectedWeightedGraph<P>, private var extra: Map<P, Collection<Edge<P>>>) : DirectedWeightedGraph<P> {
...
private fun <P> dijkstraImpl(graph: DirectedWeightedGraph<P>, start: P, isGoal: ((P) -> Boolean)?): Pair<Map<P, Edge<P>>, P?> {
...
fun <T> Map<T, Collection<Edge<T>>>.toDirectedWeightedGraph(): DirectedWeightedGraph<T> = DirectedWeightedGraphByMap<T>(this)

fun <T> XYMap<T>.toDirectedWeightedGraph(positions: Collection<Pos>, isOpen: (T) -> Boolean): DirectedWeightedGraph<Pos> {
    val topology = this.toGraph(isOpen)
    return positions.map { p1 -> p1 to (topology.getDistances(p1, positions).map { (p2, dist) -> Edge(p1, p2, dist) }) }.toMap()
...
fun <T> XYMap<T>.toDirectedWeightedGraphByContent(positions: Collection<Pos>, isOpen: (T) -> Boolean): DirectedWeightedGraph<T> {
    val topology = this.toGraph(isOpen)
    return positions.map { p1 -> this[p1] to (topology.getDistances(p1, positions).map { (p2, dist) -> Edge(this[p1], this[p2], dist) }) }.toMap()
        .toDirectedWeightedGraph()
}

fun <P> DirectedGraph<P>.getDistances(start: P, destinations: Collection<P>, continueOnPos: Boolean = false): Map<P, Int> {
...
fun <P> DirectedGraph<P>.getRoutes(start: P, destinations: Collection<P>, continueOnPos: Boolean = false): Map<P, List<P>> {
    val missing = destinations.toMutableSet()
