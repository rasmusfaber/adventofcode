...
class Day15B {
    ...
    fun part1() {
        ...
        val path = aStar(topology, ::manhattanDistance, Pos(0, 0), map.positions().first { map[it] == 2 })!!.toDirections()
        println(path.size)
    }
    ...
    fun Machine.testMove(direction: Direction): Long {
        ...
        val res = this.runUntilOutput()!!
        if (res != 0L) {
            this.sendMoveCommand(direction.opposite())
            val res2 = this.runUntilOutput()!!
